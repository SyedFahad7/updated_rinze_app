...
class HistoryOrderDetailsTab extends StatefulWidget {
  const HistoryOrderDetailsTab({
    super.key,
  ...
  @override
  State<HistoryOrderDetailsTab> createState() => _HistoryOrderDetailsTabState();
}

class _HistoryOrderDetailsTabState extends State<HistoryOrderDetailsTab> {
  ...
  List<Color> checkPaymentStatus() {
    if (widget.paymentStatus == 'paid') {
      return [
        AppColors.retroLime.withOpacity(0.3),
        AppColors.green,
      ];
  ...
  void _parseData() {
    setState(() {
      ...
      pickupAgentId = widget.pickupData['pickup_agent']?['profile']
              ?['profileRef']?['agentId'] ??
          'To be assigned';
      pickupAgentName =
          widget.pickupData['pickup_agent']?['fullName'] ?? 'To be assigned';
      pickupAgentMobileNumber = widget.pickupData['pickup_agent']
              ?['mobileNumber'] ??
          'To be assigned';
      deliveryDate =
      ...
      deliveryAgentId = widget.deliveryData['delivery_agent']?['profile']
              ?['profileRef']?['agentId'] ??
          'To be assigned';
      deliveryAgentName = widget.deliveryData['delivery_agent']?['fullName'] ??
          'To be assigned';
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4.0),
            // Wrapping ListView.builder in a constrained container
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 12.0),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Column(
                    ...
                    children: [
                      ...
                      // Loop through laundryPerPiece entries and create a grid for each category
                      if (laundryPerPiece != null)
                        ...laundryPerPiece.entries.map((entry) {
                          final category = entry.key; // e.g., "men", "women"
                                ...
                                children: [
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      Text(
                                        capitalize(
                                            formatStringToMultiline(category)),
                                        style: TextStyle(
                                          color: AppColors.hintBlack
                                              .withOpacity(0.6),
                                          fontFamily: AppFonts
                      ...
                      if (laundryByKG != null) const SizedBox(height: 16.0),

                      // Display By KG Section
                      if (laundryByKG != null)
                      ...
                      if (laundryByKG != null)
                        ...laundryByKG.entries.map((entry) {
                          ...
                          if (details.containsKey('products') &&
                            ...
                            final products =
                                details['products'] as List<dynamic>;
